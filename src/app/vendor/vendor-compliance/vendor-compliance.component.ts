...
export class VendorComplianceComponent implements OnInit {
  ...
  get_data() {
    ...
    this.crudService.post_request(this.get_single, GlobalComponent.getCompliance)
      .subscribe(({
        next: (response) => {
  ...
  upload_id() {
    ...
    if (this.compliance.front.length < 40) {
      this.error_notification("ID Front and Back is required");
      return;
    }
    ...
    this.ui_controls.is_loading = true;
    this.crudService.post_request(this.compliance, GlobalComponent.updateCompliance)
      .subscribe(({
        next: (response) => {
          if (response.response_code === 200 && response.status === "success") {
            this.ui_controls.is_loading = false;
  ...
  async select_front(event: any) {
    ...
    try {
      ...
      const compressedFile = await imageCompression(file, {
        ...
        maxWidthOrHeight: 650, // max width/height
        useWebWorker: true,
      });
      ...
      reader.onloadend = () => {
        ...
        this.compliance.front = this.base64String;
      };
  ...
  async select_back(event: any) {
    ...
    try {
      ...
      reader.onloadend = () => {
        this.base64String = reader.result as string;
        this.compliance.back = this.base64String;
      };
      reader.readAsDataURL(compressedFile);
  ...
  async select_license_doc(event: any) {
    ...
    try {
      ...
      reader.onloadend = () => {
        this.base64String = reader.result as string;
        this.compliance.license_doc = this.base64String;
      };
      reader.readAsDataURL(compressedFile);
