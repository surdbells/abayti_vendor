...
export class CreateProductComponent implements OnInit{
  ...
  start_publish(status: string): void {
    if (this.create.category === 0) {
      this.error_notification("Product category is required");
      return;
    }
    ...
    if (this.create.description.length === 0) {
      this.error_notification("briefly describe your product");
      return;
    }
    if (this.create.image_1.length === 0) {
      this.error_notification("Featured image is required");
      return;
    }
    if (this.create.delivery_time.length === 0) {
      this.error_notification("Delivery time is required");
      return;
    }
    this.dialogs
